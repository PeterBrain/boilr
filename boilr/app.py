...
class Boilr:
    ...
    def __init__(
        ...
        self.pload = pload \
            or deque(maxlen=config.SystemConfig.moving_median_list_size)
        self.ppv = ppv \
            or deque(maxlen=config.SystemConfig.moving_median_list_size)
        self.pload_median = 0
        self.ppv_median = 0
    ...
    def update_medians(self, powerflow_pload, powerflow_ppv):
        ...
        try:
            ...
            logger.debug(
                "Median power pv: %s W",
                round(self.ppv_median, 2)
            )
            logger.debug(
                "Median power load: %s W",
                round(self.pload_median, 2)
            )
            '''logger.debug(
                "Power load deque: %s",
                list(self.pload)
...
def run():
    ...
    """
    # check date range
    (boilr.date_check, date_check_msg) = \
        helper.date_check(config.SystemConfig.active_date_range)
    ...
    try:
        ...
        if not rpi_gpio.gpio_mode(
            ...
            if boilr.status_prev[0] or \
                (not boilr.status_prev[0] and
                    (boilr.status_prev[1] < datetime.now()
                        - timedelta(
                            seconds=config.SystemConfig.start_timeout
                        ))):
                ...
                if boilr.status_prev[0] != boilr.status[0]:
                    ...
                    logger.info(
                        "Status: %s",
                        "active" if boilr.status[0] else "inactive"
                    )
...
def manual_override(args):
    ...
    try:
        if (not rpi_gpio.gpio_mode(
                config.RpiConfig.rpi_channel_relay_out,
                "out"
            ) or not rpi_gpio.gpio_mode(
                    config.RpiConfig.rpi_channel_relay_in,
                    "in"
                )):
        ...
        if args in {0, 1}:
            logger.debug(
                ...
                "closed" if args == 1 else "open"
            )
            logger.info(
                "Status: %s (manual)",
                "active" if args == 1 else "inactive"
            )
            if not rpi_gpio.output_relay(
                config.RpiConfig.rpi_channel_relay_out,
