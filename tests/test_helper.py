...
def mock_datetime_now():
    ...
    with patch('boilr.helper.datetime') as mock_datetime:
        mock_datetime.now.return_value = datetime(2024, 6, 15)  # Mocking to 15th June 2024
...
@pytest.fixture
def mock_datetime_now_time():
    """Fixture to mock datetime.now() to return specific time."""
    with patch('boilr.helper.datetime') as mock_datetime:
        mock_datetime.now.return_value = datetime(2024, 6, 15, 15, 30)  # Mocking to 15:30
        mock_datetime.strptime = datetime.strptime
        yield mock_datetime

def test_time_check_within_range(mock_datetime_now_time):
...
def test_time_check_out_of_range(mock_datetime_now_time):
